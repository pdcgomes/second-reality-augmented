...
// MODIFIED TO ADJUST BPM (GLOBAL_MUSIC_SPEED_GAIN)
...
Screamtracker.prototype.advance = function(mod) {
  let bpm=mod.bpm*GLOBAL_MUSIC_SPEED_GAIN;   //slight tempo change to exactly fit original Second reality music timing
  mod.stt=(((mod.samplerate*60)/bpm)/4)/6; // samples to tick
...
Screamtracker.prototype.mix = function(mod, bufs, buflen) {
  ...
  for(var s=0;s<buflen;s++)
  {
    ...
    for(var ch=0;ch<mod.channels;ch++)
    {
      var fl=0.0; var fr=0.0; var fs=0.0;
