...
export function buildTimeMap(arrayBuffer, speedGain) {
  ...
  var rowSpeeds = new Uint8Array(256 * 64);
  var rowBPMs = new Uint8Array(256 * 64);
  ...
  rowTimes[0] = 0;
  rowSpeeds[0] = player.speed;
  rowBPMs[0] = player.bpm;
  ...
  while (totalSamples < maxSamples) {
    ...
    if (player.endofsong) {
      ...
      if (rowTimes[endKey] < 0) {
        ...
        rowSpeeds[endKey] = player.speed;
        rowBPMs[endKey] = player.bpm;
      }
      break;
    }
    ...
    if (pos !== prevPos || row !== prevRow) {
      ...
      if (rowTimes[key] < 0) {
        ...
        rowSpeeds[key] = player.speed;
        rowBPMs[key] = player.bpm;
      }
      prevPos = pos;
      prevRow = row;
    }
...
Modplayer.prototype.play = function () {
  ...
  if (!this.gainNode) this.createContext();
  this.player.samplerate = this.samplerate;
  this.player.speedGain = this.speedGain;
  if (this.player.paused) {
    this.player.paused = false;
    this._playCtxTime = this.context.currentTime - this._pauseElapsed;
    this._nextSchedTime = this.context.currentTime;
  ...
  this.playing = true;
  this._samplesProcessed = 0;
  this._playCtxTime = this.context.currentTime;
...
Modplayer.prototype.pause = function () {
  if (this.player && !this.player.paused) {
    ...
    this._pauseElapsed = this.context ? this.context.currentTime - this._playCtxTime : 0;
    this._stopScheduler();
  }
};
...
Modplayer.prototype.seek = function (position, row, speed, bpm) {
  if (this.player) {
    this._flushSources();
    this.player.tick = 0;
    this.player.row = row;
    this.player.position = position;
    if (speed !== undefined) this.player.speed = speed;
    if (bpm !== undefined) this.player.bpm = bpm;
    this.player.flags = 1 + 2;
    this._samplesProcessed = 0;
...
// ── Audio context + buffer-scheduling setup ──────────────────────────
...
Modplayer.prototype.createContext = function () {
  ...
  this._chunkSize = Math.round(this.samplerate * 0.1);
  this._lookAhead = 0.4;
  this._nextSchedTime = 0;
  this._sources = [];
...
Modplayer.prototype.reconnectAudio = function (newCtx) {
  this._stopScheduler();
  ...
  this._nextSchedTime = this.context.currentTime;
  this._sources = [];
  if (this.player) this.player.samplerate = this.samplerate;
...
// ── Buffer scheduling engine ─────────────────────────────────────────
//
// Replaces ScriptProcessorNode with pre-rendered AudioBufferSourceNodes.
// Chunks of ~100ms are rendered by the S3M mixer and scheduled via the
// Web Audio clock for gapless playback. AudioBufferSourceNode is
// universally supported on mobile browsers.

Modplayer.prototype._startScheduler = function () {
  if (this._intervalId) return;
  var mod = this;
  this._scheduleChunks();
  this._intervalId = setInterval(function () { mod._scheduleChunks(); }, 50);
};

Modplayer.prototype._stopScheduler = function () {
  if (this._intervalId) { clearInterval(this._intervalId); this._intervalId = null; }
  this._flushSources();
};

Modplayer.prototype._flushSources = function () {
  if (!this._sources) return;
  for (var i = 0; i < this._sources.length; i++) {
    try { this._sources[i].src.stop(); } catch (_) {}
  }
  this._sources = [];
};

Modplayer.prototype._scheduleChunks = function () {
  if (!this.player || !this.player.playing || this.player.paused) return;
  if (!this.context || !this.gainNode) return;

  var now = this.context.currentTime;

  // Update _samplesProcessed from actual playback position
  this._samplesProcessed = Math.max(
    this._samplesProcessed,
    Math.floor((now - this._playCtxTime) * this.samplerate)
  );

  // Prune finished sources
  var live = [];
  for (var i = 0; i < this._sources.length; i++) {
    if (this._sources[i].end > now) live.push(this._sources[i]);
  }
  this._sources = live;

  // Render and schedule chunks until we have enough buffered
  while (this._nextSchedTime < now + this._lookAhead) {
    this.player.repeat = this.repeat;
    var chunkSize = this._chunkSize;
    var bufs = [new Float32Array(chunkSize), new Float32Array(chunkSize)];
    this.player.mix(this.player, bufs, chunkSize);

    // Stereo separation + soft clipping (same as the old callback)
    var outp = new Float32Array(2);
    for (var s = 0; s < chunkSize; s++) {
      outp[0] = bufs[0][s]; outp[1] = bufs[1][s];
      if (this.separation) {
        var t = outp[0];
        if (this.separation === 2) { outp[0] = outp[0] * 0.5 + outp[1] * 0.5; outp[1] = outp[1] * 0.5 + t * 0.5; }
    ...
    this.reachedBoundary = this.player.reachedStop || false;

    // Create AudioBuffer and schedule it
    var ab = this.context.createBuffer(2, chunkSize, this.samplerate);
    ab.getChannelData(0).set(bufs[0]);
    ab.getChannelData(1).set(bufs[1]);
    var src = this.context.createBufferSource();
    src.buffer = ab;
    src.connect(this.gainNode);
    src.start(this._nextSchedTime);

    var end = this._nextSchedTime + chunkSize / this.samplerate;
    this._sources.push({ src: src, end: end });
    this._nextSchedTime = end;

    // VU meters
    for (var c = 0; c < this.player.channels; c++) {
      this.chvu[c] = this.chvu[c] * 0.25 + this.player.chvu[c] * 0.75;
      this.player.chvu[c] = 0;
    }

    if (this.endofsong && this.playing) { this.stop(); break; }
    if (this.reachedBoundary) break;
  }
};
