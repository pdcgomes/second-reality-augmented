...
Modplayer.prototype.createContext = function () {
  ...
  var mod = this;
  this.mixerNode.onaudioprocess = function (ape) {
    Modplayer.prototype._doMix(mod, ape);
  };
  this.mixerNode.connect(this.gainNode);

  try {
    var sb = this.context.createBuffer(1, 1, this.context.sampleRate);
...
Modplayer.prototype.reconnectAudio = function (newCtx) {
  if (this.mixerNode) {
    try { this.mixerNode.disconnect(); } catch (_) {}
    this.mixerNode.onaudioprocess = null;
  }
  if (this.gainNode) {
...
Modplayer.prototype._doMix = function (mod, ape) {
...
// Legacy entry point (unused in closure-based path but kept for compat)
Modplayer.prototype.mix = function (ape) {
  var mod = (ape.target && ape.target.module) || (ape.srcElement && ape.srcElement.module) || (this && this.module);
  if (mod) Modplayer.prototype._doMix(mod, ape);
};
